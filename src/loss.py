...
class lovasz_softmax(nn.Module):
  def __init__(self,classes=[1], per_image=False, ignore=None):
  ...
  def forward(self, probas, labels):
    if self.per_image:
        loss = mean(lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), self.ignore), classes=self.classes)
                          for prob, lab in zip(probas, labels))
    else:
...
class binary_xloss(nn.Module):
  ...
  def forward(self, logits, labels):
    logits, labels = flatten_binary_scores(logits, labels, self.ignore)
    loss = StableBCELoss()(logits, Variable(labels.float()))
    return loss
###############################################################################
# Focal Loss#
